...
dados_roe <- dados_selecionados %>%
  ...
  filter(!is.na(ROE)) %>%
  mutate(
    Empresa = paste0(emp, ' (', Estado, ')\n',
                     'Dependência: ', dep, '\n',
                     'Lucros: ', format(lucros, big.mark = '.', decimal.mark = ","), '\n',
                     'PL: ', PL_formatado, '\n',
...
sumario_roe_gov <- dados_roe %>%
  ...
  summarise(qde = n(), .groups = "drop_last") %>%
  mutate(pct_qde = percent(qde / sum(qde))) %>%  # % dentro de cada grupo de gov
  ungroup() %>%
  mutate(
    y = case_when(
      cat_ROE == "bem_neg" ~ -0.75,
      cat_ROE == "neg"     ~ -0.25,
      cat_ROE == "pos"     ~  0.25,
      cat_ROE == "bem_pos" ~  0.75
    )
...
# (apenas para o espalhamento, limito a faixa do ROE)
roe_gov <- ggplot(
  dados_roe %>%
    filter(!is.na(ROE),
           ROE != 0,
           ...
           !is.na(lucros),
           lucros != 0),
  aes(x = gov, y = ROE, color = sinal_ROE, label = Empresa)
) +
  geom_quasirandom() +
  scale_color_manual(values = c("Negativo" = "#DC143C",
                                "Positivo" = "#008080")) +
  annotate("rect", xmin = 0,   xmax = 1.5, ymin = 0, ymax = 2,
           alpha = 0.2, fill = "antiquewhite") +
  annotate("rect", xmin = 1.5, xmax = 2.7, ymin = 0, ymax = 2,
           alpha = 0.2, fill = "antiquewhite") +
  geom_text(
    data = sumario_roe_gov_sinal,
    aes(
      y     = ifelse(!gov, y, NA_real_),
      label = paste0(pct_qde,
                     ' das que NÃO \npossui estrutura \nde governança'),
      color = sinal_ROE
    ),
    x       = 0.9,
  ...
  scale_x_discrete(labels = c(
    `FALSE` = "Empresas que NÃO possuem\nestrutura de Governança completa",
    `TRUE`  = "Empresas que possuem\nestrutura de Governança completa"
  )) +
  tema()
