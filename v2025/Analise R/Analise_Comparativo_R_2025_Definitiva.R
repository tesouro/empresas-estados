...
dados_raw_2024 <- read_excel("C:/Users/gustavo.silva/OneDrive - Tesouro Nacional/Área de Trabalho/projetos/RAIO_X_EMPRESAS_DOS_ESTADOS/2023/dados/dados-originais/Dados_para_Raio-x2_(compatibilizando_com_quadro_estatais_v3).xlsx", sheet = "ex2024") # nolint
...
# --- Versão tratada de 2024 (corrigida) ---------------------------------
dados_selecionados2024 <- dados_selecionados_raw_2024 %>%
  ...
  # numéricos que já vêm como número: apenas coerção simples
  mutate(
    PL               = as.numeric(PL),
    lucros           = as.numeric(lucros),
    desp_investimento= as.numeric(desp_investimento),
    desp_pessoal     = as.numeric(desp_pessoal),
    qde_empregados   = as.numeric(qde_empregados)
  ) %>%
  # colunas que vêm como texto com ponto/milhar e vírgula/decimal: usar num_pt
  mutate(across(
    c(Dividendos, `Subvenção`, `Subvenção (anterior)`,
...
situacao_por_ano <- dados_empresas %>%
  mutate(
    # Normaliza situação INATIVO como EM LIQUIDAÇÃO
    sit = if_else(str_to_upper(sit) == "INATIVO", "EM LIQUIDAÇÃO", sit),

    # Cria versões normalizadas (maiúsculas) para comparação
    dep_norm = str_to_upper(dep),
    sit_norm = str_to_upper(sit),

    # Monta categoria combinando situação e dependência
    sit_dep = case_when(
      sit_norm == "ATIVA"         & dep_norm == "NÃO DEPENDENTE" ~ "ATIVA - não dependente",
      sit_norm == "ATIVA"         & dep_norm == "DEPENDENTE"     ~ "ATIVA - dependente",
      sit_norm == "EM LIQUIDAÇÃO" & dep_norm == "NÃO DEPENDENTE" ~ "EM LIQUIDAÇÃO - não dependente",
      sit_norm == "EM LIQUIDAÇÃO" & dep_norm == "DEPENDENTE"     ~ "EM LIQUIDAÇÃO - dependente",
      TRUE ~ NA_character_
    ),
    # Garante níveis fixos e ordem desejada
    sit_dep = factor(
      sit_dep,
      levels = c(
        "ATIVA - não dependente",
        "ATIVA - dependente",
        "EM LIQUIDAÇÃO - não dependente",
        "EM LIQUIDAÇÃO - dependente"
      )
    )
  ) %>%
  # Remove combinações indefinidas (NA)
  filter(!is.na(sit_dep)) %>%
  group_by(Ano, sit_dep) %>%
  summarise(Quantidade = n(), .groups = "drop")

# Paleta de cores para cada situação/dependência
...
situacao_empresas <- ggplot(
  situacao_por_ano,
  aes(x = as.factor(Ano), y = Quantidade, fill = sit_dep)
) +
  ...
  scale_fill_manual(values = cores_personalizadas, drop = FALSE) +
  theme_minimal() +
  theme(
...
ggsave("./plots_final/situacao_empresas.jpeg",
       plot = situacao_empresas, width = 15, height = 15)
...
grafico_lucros <- ggplot(lucros_por_ano2,
                         aes(x = Ano, y = Valor, color = Tipo, group = Tipo)) +
  ...
  geom_text(
    aes(
      label = formatar_valor(Valor),
      vjust = ifelse(Valor >= 0, -0.6, 1.6)  # acima p/ lucro, abaixo p/ prejuízo
    ),
    size   = 6,
  ...
  scale_x_continuous(
    breaks = unique(lucros_por_ano2$Ano),
    expand = expansion(mult = c(0.02, 0.08))  # folga p/ não cortar à direita
  ) +
  ...
  coord_cartesian(clip = "off")  # evita cortar os rótulos

ggsave("./plots_final/evolucao_lucros_prejuizos.jpeg",
       grafico_lucros, width = 10, height = 7, device = "jpeg")


# Distribuição de PLR por Ano (todas as empresas) -------------------------

pl_distribuicao <- dados_empresas %>%
  # Usar apenas empresas com informação clara de PLR/RVA
  mutate(plr_flag = case_when(
    plr_rva %in% c("Sim", "SIM")               ~ "Distribuído",
    plr_rva %in% c("Não", "NÃO", "NAO")        ~ "Não Distribuído",
    TRUE                                       ~ NA_character_
  )) %>%
  filter(!is.na(plr_flag)) %>%                      # exclui vazios / outros
  group_by(Ano, plr_flag) %>%
  summarise(qtd_empresas = n(), .groups = "drop") %>%
  group_by(Ano) %>%
  mutate(pct = qtd_empresas / sum(qtd_empresas)) %>%   # % de empresas por ano
  ungroup()

ggplot(pl_distribuicao, aes(x = "", y = pct, fill = plr_flag)) +
  ...
  geom_text(aes(label = scales::percent(pct, accuracy = 0.1)),
            ...
            size = 15, color = "white") +
  scale_fill_manual(
    values = c("Distribuído" = "#735D36",
               "Não Distribuído" = "#F4C773"),
    name = "PLR/RVA"
  ) +
...
ggsave("./plots_final/distribuicao_PLR.jpeg",
       width = 15, height = 10)
...
# Normaliza informação de conselhos e cria flag "todas_conselhos" ---------

sim_gov  <- c("SIM", "Sim")  # apenas "Sim"

dados_gov <- dados_empresas %>%
  mutate(
    dep_norm = stringr::str_to_upper(dep),

    ca_norm  = stringr::str_to_upper(trimws(as.character(gov_ca))),
    cf_norm  = stringr::str_to_upper(trimws(as.character(gov_cf))),
    aud_norm = stringr::str_to_upper(trimws(as.character(gov_aud))),

    # Regra espelhando a planilha: SIM só se os 3 forem SIM; todo o resto é "NÃO"
    todas_conselhos = dplyr::if_else(
      ca_norm  %in% sim_gov &
      cf_norm  %in% sim_gov &
      aud_norm %in% sim_gov,
      "SIM",
...
dados_dep <- dados_gov %>%
  dplyr::filter(dep_norm == "DEPENDENTE")

dados_resumo_dep <- dados_dep %>%
  dplyr::count(todas_conselhos, name = "frequencia") %>%
  dplyr::mutate(
    grupo = ifelse(
      todas_conselhos == "SIM",
      "Possui todas as\n3 governança",
    ...
    pct = frequencia / sum(frequencia)
  )

grafico_pizza_dep <- ggplot(dados_resumo_dep,
                            aes(x = "", y = pct, fill = grupo)) +
...
ggsave(plot = grafico_pizza_dep,
       filename = "./plots_final/pizza_dep_possui.png",
       width = 4, height = 3, dpi = 300)

# Pizza (NÃO DEPENDENTE) governança ---------------------------------------

dados_ndep <- dados_gov %>%
  dplyr::filter(dep_norm == "NÃO DEPENDENTE")

dados_resumo_ndep <- dados_ndep %>%
  dplyr::count(todas_conselhos, name = "frequencia") %>%
  dplyr::mutate(
    grupo = ifelse(
      todas_conselhos == "SIM",
...
grafico_pizza_ndep <- ggplot(dados_resumo_ndep,
                             aes(x = "", y = pct, fill = grupo)) +
  geom_col(width = 1, color = NA) +
  coord_polar(theta = "y", start = 0) +
  geom_text(aes(label = scales::percent(pct, accuracy = 0.1)),
...
dir.create("./plots_final", showWarnings = FALSE, recursive = TRUE)
ggsave(plot = grafico_pizza_ndep,
...
stopifnot(exists("dados_selecionados_raw_2024"),
          exists("dados_selecionados_raw_2023"))

c23 <- get_cnpj_col(dados_selecionados_raw_2024)
c22 <- get_cnpj_col(dados_selecionados_raw_2023)
